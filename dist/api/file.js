...
const changeCase = arrayData => arrayData.map((ele, index) => index % 2 === 0 ? ele.toUpperCase() : ele.toLowerCase());
...
readFile('./readme.txt').then(splitByNewLine).then(changeCase).then(mergeEleInArray).then(writeDataToFile).then(console.log).catch(r => console.log('error - ' + r));